...
Read from stores:
- `pipelineConfig` — research topic, target project, task mode, output types, subject type
...
- Code blocks have language tags (` ```solidity `, ` ```json `, ` ```typescript `, etc. — not bare ` ``` `)
...
5. **Verification tier labels** — per `global-constraints.md`, every quantitative claim must carry a tier label. The expanded tier set is: `[Source code verified]`, `[On-chain verified]`, `[Benchmark verified]`, `[Scorecard data]`, `[Platform data]`, `[Docs claim]`, `[Changelog verified]`, `[Independent review]`, `[Third-party claim]`, `[Vendor benchmark]`, `[Community signal]`, `[Inference]`, `[Unverified]`. Flag claims missing labels.
...
Compute a structured quality assessment:

1. **sourceCount**: Count all unique URLs cited inline across all deliverables (deduplicate exact URLs)
2. **verifiedClaimsRatio**: From `factCheckResults`, compute `"{verifiedClaims.length + failedClaims.length that were corrected}/{claimsChecked}"` — this is the proportion of checked claims that are now confirmed correct
3. **tier1FactRatio**: Scan deliverables for tier labels. Count facts tagged `[Source code verified]`, `[On-chain verified]`, `[Benchmark verified]` (Tier 1-3) vs total labeled facts. Report as `"{tier1_3_count}/{total_labeled}"`
4. **gapCount**: Count claims tagged `[Unverified]` or `[Inference]` plus any "UNVERIFIABLE" items from `factCheckResults`
5. **confidenceScore**: Inherit from `factCheckResults.confidenceScore`, then adjust:
   - Downgrade by one level if `completenessPassed == false`
   - Downgrade by one level if `factCheckApplied == false`
   - Cannot go below `low`
6. **grade**: Based on the combination:
   - **A** (publish-ready): confidenceScore == "high" AND formattingPassed AND completenessPassed AND factCheckApplied AND gapCount <= 3
   - **B** (minor issues): confidenceScore >= "medium" AND completenessPassed AND gapCount <= 8
   - **C** (significant gaps): confidenceScore == "medium" OR gapCount <= 15
   - **D** (major rework needed): everything else
...
- `qualityScorecard`: the quality metrics computed in Step 6
...
```json
{
  "finalOutput": {
    ...
    "qualityScorecard": {
      "sourceCount": 0,
      "verifiedClaimsRatio": "42/50",
      "tier1FactRatio": "35/50",
      "gapCount": 0,
...
- `qualityScorecard`: structured quality metrics — `sourceCount` (unique URLs cited), `verifiedClaimsRatio` (confirmed-correct claims / total checked), `tier1FactRatio` (high-tier sourced facts / total), `gapCount` (unverified or inference-only claims), `confidenceScore` (inherited from factCheck, adjusted by editorial), `grade` (A = publish-ready, B = minor issues, C = significant gaps, D = rework needed)
