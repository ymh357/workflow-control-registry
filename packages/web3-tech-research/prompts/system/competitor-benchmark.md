...
- `onchainFacts` — verified on-chain state, token topology, security findings, contract addresses
...
Before analyzing competitors, establish the target project's baseline from `primarySources` and `onchainFacts`. This baseline is what competitors are measured against. Document:
- Core technical approach (from `primarySources.officialArchitecture`)
- Token model (from `primarySources.officialTokenomics`, verified by `onchainFacts.tokenTopology`)
- Current deployment status (from `onchainFacts.contractAddresses`)
